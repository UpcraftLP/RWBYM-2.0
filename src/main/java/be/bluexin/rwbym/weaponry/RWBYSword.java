...
public class RWBYSword extends ItemSword implements ICustomItem {
    ...
    public ActionResult<ItemStack> onItemRightClick( World worldIn, EntityPlayer playerIn, EnumHand hand) {
        ...
        if (!worldIn.isRemote && playerIn.isSneaking() && this.morph != null) {
            ...
            /*else if (crescentr && hand == EnumHand.MAIN_HAND && !playerIn.onGround) {
            ...
            /*if(worldIn.isRemote){
                ...
                nbt.setBoolean("runhideevent", true);
                nbt.setInteger("timer", 20);
                is.setTagCompound(nbt);
            }
            runhideevent = true;
            timer = 20;
            if (!flag1) {
                is.damageItem(-100, playerIn);
            }
                for (int i = 0; i < 32; ++i) {
                    RWBYModels.proxy.generateRosepetals(playerIn);
                }
            if (!flag1) {
                is.damageItem(20, playerIn);
            }
        }*/}

        /*public void Timer(){
        timer -=1;
        if(timer ==0){runhideevent = false;}}*/
